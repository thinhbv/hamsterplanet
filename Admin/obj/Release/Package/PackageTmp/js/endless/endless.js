...
$(function	()	{
	...
	$('.upload-demo').change(function () {
		var filetypeallow = ["image/jpeg", "image/png", "image/gif", "application/msword", "application/vnd.ms-excel", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "application/zip", "text/plain"];
		var filename;
		var files = $(this)[0].files;
		if (files.length > 0) {
			if (filetypeallow.includes(files[0].type) === false) {
				alert("File [" + files[0].name + "] không hợp lệ.");
				return false;
			}
			filename = files[0].name;
			for (var i = 1; i < files.length; i++) {
				if (filetypeallow.includes(files[i].type) === false) {
					alert("File [" + files[i].name + "] không hợp lệ.");
					return false;
				}
			...
			$(this).parent().find('span').attr('data-title', 'No file selected...');
		}
